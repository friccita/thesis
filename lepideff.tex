...
The tight muon ID used in the trigger muon selection is one of the standard ones used by the CMS experiment. A data/MC efficiency scale factor error of 0.5\%, obtained from studies measuring the tight muon ID efficiency in $Z\rightarrow\mu\mu$ events~\cite{Chatrchyan:2012xi}, is used in the exclusion limit calculation (cf. Secs.~\ref{sec:results-systematics} and~\ref{sec:results-interpretation}).

%HLT uncertainty
\subsection{HLT\label{lepideff-HLT}}

For the WH and ZH signals, the error on the \texttt{HLT\_IsoMu24\_eta2p1} efficiency data/MC scale factor is taken as 0.2\%, as measured from single-muon HLT efficiency studies in $Z\rightarrow\mu\mu$ events~\cite{CMS:muonhlttwiki}.  As the standard scale factors were computed for isolated $Z$ decay muons, they are applicable to the isolated $W$($Z$) decay muons present in the WH(ZH) signal samples.
...
For the WH and ZH signals, the error on the PF relative isolation efficiency data/MC scale factor is taken as 0.2\%, a standard value measured in studies of $Z\rightarrow\mu\mu$ events in CMS~\cite{Chatrchyan:2012xi}.  As the standard scale factors were computed for isolated $Z$ decay muons, they are applicable to the isolated $W$($Z$) decay muons present in the WH(ZH) signal samples.
