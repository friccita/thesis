...
\item \textbf{Muon trigger efficiency: } As measured in dedicated single-muon HLT efficiency studies~\cite{CMS:muonuncertaintytwiki} (see Section~\ref{lepideff-HLT}), the systematic uncertainty from the single-muon trigger \texttt{HLT\_IsoMu24\_eta2p1} is 0.2\% for the WH and ZH signals. For the ggH and VBF signals, because of the effect of the nearby lepton filter applied to the trigger muon, a larger systematic uncertainty of 4.2\% is applied (see Sec.~\ref{lepideff-HLT} for details).
\item \textbf{Tight muon ID efficiency: } Using the results from muon ID efficiency studies~\cite{CMS:muonuncertaintytwiki} (see Section~\ref{lepideff-tightID}), the systematic uncertainty on the trigger muon tight ID is 0.5\%.
\item \textbf{Muon isolation efficiency: } The systematic uncertainty on the trigger muon isolation is taken to be 0.2\%~\cite{CMS:muonuncertaintytwiki}, as measured in the muon ID efficiency studies (see Section~\ref{lepideff-iso}). This is applied to signal events in the WH and ZH channels. For the ggH and VBF channels, an uncertainty of 10\% is used instead, to account for the fact that the muon which fires the trigger comes from a boosted $\tau_{\mu}\tau_{\text{had}}$ topology, and that the isolation efficiency for the trigger muon is largely recovered if the nearby reconstructed tau is subtracted from its isolation cone; the 10\% figure is taken from the CMS recommendation for the HPS tau ID efficiency for this boosted configuration (see Section~\ref{sec:HPS-id} for details).
\item \textbf{Soft muon ID efficiency: } According to muon ID efficiency measurements in $J/\psi\rightarrow\mu\mu$ events at CMS~\cite{CMS:muonuncertaintytwiki,CMS:muonrefeffstwiki}, the systematic uncertainty on the $\tau_{\mu}$ ID is 1.5\%.
\item \textbf{HPS ID efficiency: } The accepted value of 6\% is used~\cite{CMS:tauuncertaintytwiki}, as measured from $\tau_{had}$ ID efficiency studies~\cite{Tau14001} (see Section~\ref{sec:HPS-id}).
\item \textbf{Tau charge misidentification rate: } The accepted value of -1\%/+2\% from CMS $\tau$ ID efficiency studies is used~\cite{CMS:tauuncertaintytwiki}.
...
\item \textbf{$M_{\text{T}}$: } Using calculations done on our signal samples (calculation procedures described at~\cite{CMS:btaguncertaintytwiki}), errors range up to 12.2\% depending on the pseudoscalar mass and production channel. Just as for the b-veto errors, the MET errors calculated for each mass point in the ggH channel are applied to the VBF prediction for each analogous mass point, while the errors calculated for the WH channel are applied to the ZH channel.
...
\begin{figure}[hbtp]
  \begin{center}
    \includegraphics[width=1.24\cmsFigWidth]{figures/expLimits_Br_lowMT_20GeV_ggHVBF}
    \includegraphics[width=1.24\cmsFigWidth]{figures/expLimits_Br_highMT_20GeV_ggHWH}
    \includegraphics[width=1.24\cmsFigWidth]{figures/expLimits_Br_20GeV_4sig}
