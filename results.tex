...
Various statistical techniques exist for assessing the compatibility of observed data with the background-only (or null) hypothesis and the signal + background hypothesis. With some techniques, one can set a confidence limit on the parameter under study, such as the signal process cross section, beyond which one can exclude the signal hypothesis with a desired level of confidence (the conventional confidence level 95\% is used in this search).
